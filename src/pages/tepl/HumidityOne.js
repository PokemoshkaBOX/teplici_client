...
import {getHumidity} from "../../http/teplAPI";
Chart.register(
  BarElement,
  CategoryScale,
  LinearScale,
...
const HumidityOne= ({dayfirst, daylast, type, name}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getHumidity(dayfirst, daylast, type);
        console.log(data)
        setFetchedData(data);
      } catch (error) {
